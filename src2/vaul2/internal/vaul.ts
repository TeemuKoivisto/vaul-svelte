...
export function createVaul(props: CreateVaulProps) {
	...
	let drawerHeightRef = drawerRef.get()?.getBoundingClientRect().height || 0;
	...
	function onPress(event: SvelteEvent<PointerEvent, HTMLElement>) {
		...
		pointerStart = isVertical(direction.get()) ? event.screenY : event.screenX;
	}
	...
	function shouldDrag(el: EventTarget, isDraggingInDirection: boolean) {
		...
		const $scrollLockTimeout = scrollLockTimeout.get();
	...
	function onDrag(event: SvelteEvent<PointerEvent | TouchEvent, HTMLElement>) {
		...
		const $activeSnapPointIndex = activeSnapPointIndex.get();
		const $snapPoints = snapPoints.get();

		// Disallow dragging down to close when first snap point is the active one and dismissible prop is set to false.
		if ($snapPoints && $activeSnapPointIndex === 0 && !dismissible.get()) return;
		...
		const $overlayRef = overlayRef.get();
		...
		const $fadeFromIndex = fadeFromIndex.get();
		const $shouldFade = shouldFade.get();
		...
		if (wrapper && $overlayRef && shouldScaleBackground.get()) {
	...
	function scaleBackground(open: boolean, backgroundColor: string | undefined = "black") {
		...
		if (!wrapper || !shouldScaleBackground.get()) return;
		const $direction = direction.get();
	...
	function resetDrawer() {
		...
		const $shouldScaleBackground = shouldScaleBackground.get();
	...
	function onRelease(event: SvelteEvent<PointerEvent | MouseEvent | TouchEvent, HTMLElement>) {
		...
		const visibleDrawerHeight = Math.min(
			drawerRef.get()?.getBoundingClientRect().height ?? 0,
			window.innerHeight
		);

		if (swipeAmount >= visibleDrawerHeight * closeThreshold.get()) {
			closeDrawer();
			onReleaseProp?.(event, false);
			return;
		}
