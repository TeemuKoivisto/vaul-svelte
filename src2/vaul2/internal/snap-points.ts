...
import { effect, set, isVertical, isBottomOrRight } from "./helpers/index";
...
export function handleSnapPoints({
	...
	function onRelease({
		...
		const $activeSnapPointOffset = activeSnapPointOffset.get();
		...
		if (velocity > VELOCITY_THRESHOLD && Math.abs(draggedDistance) < dim * 0.4) {
			...
			if (dragDirection > 0 && isLastSnapPoint.get() && $snapPoints) {
	...
	function getPercentageDragged(absDraggedDistance: number, isDraggingDown: boolean) {
		...
		if (!shouldFade.get() && !isOverlaySnapPoint) return null;
